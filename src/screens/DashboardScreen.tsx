        ...
        <motion.div className="px-5 pb-6" variants={containerVariants} initial="hidden" animate="visible">
          ...
          <motion.div variants={cardVariants} className="rounded-2xl bg-white border border-border p-5 mb-4 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-lg font-bold text-primary">{mockData.launchProgress}%</span>
            </div>
            <div className="h-2.5 bg-muted rounded-full overflow-hidden mb-3">
              <motion.div className="h-full bg-gradient-to-r from-primary to-[#E8956D] rounded-full" initial={{
              width: 0
            }} animate={{
              width: `${mockData.launchProgress}%`
            }} transition={{
              duration: 0.8,
              ease: "easeOut"
            }} />
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            ...
            <motion.div variants={cardVariants} className="rounded-2xl bg-white border border-border p-4 shadow-sm border-dotted">
            ...
            <motion.div variants={cardVariants} className="rounded-2xl bg-white border border-border p-4 shadow-sm border-dotted">
              ...
              <p className="text-xs text-muted-foreground">Step {mockData.legal.current} of {mockData.legal.total}</p>
            </motion.div>
          </div>
          ...
          <motion.div variants={cardVariants} className="rounded-2xl bg-[hsl(25,30%,88%)] p-5 mb-4">
            ...
            <p className="text-sm text-secondary/70 mb-4">Connect with {mockData.community.members.toLocaleString()}+ entrepreneurs who've scaled in {mockData.community.category}<br />​
            </p>
